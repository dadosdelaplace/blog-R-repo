...
```{r echo = FALSE, out.width = "60%", fig.align = "center", fig.cap = "Wordle, el juego de moda: <https://www.powerlanguage.co.uk/wordle/>"}
knitr::include_graphics("./wordle.jpg")
```
...
```{r echo = FALSE, out.width = "60%", fig.align = "center", fig.cap = "Wordle, el juego de moda: <https://www.powerlanguage.co.uk/wordle/>"}
knitr::include_graphics("./wordle1.jpg")
```
...
```{r echo = FALSE, out.width = "60%", fig.align = "center", fig.cap = "Wordle, el juego de moda: <https://www.powerlanguage.co.uk/wordle/>"}
knitr::include_graphics("./wordle2.jpg")
```
...
¿Y cuántas palabras hay de 3, 5 o 9 letras?
...
ggplot(datos_CREA_filtrado,
  ...
  labs(y = glue("Frec. absoluta ({n_palabras_CREA} palabras)"),
       ...
       paste0("Distribución del nº letras de CREA. Se han eliminado las que\n",
              "aparecen en menos de 5 de cada 10 000 (152 560 docs analizados)"),
       caption =
         paste0("Javier Álvarez Liébana (@dadosdelaplace) | Datos: CREA"))
...
En los datos anteriores se han incluido **todas las palabras del CREA** que superan cierto número de repeticiones en los documentos (al menos aparecer en 1 de cada 10 000 documentos), y vemos como de **5 letras tan solo contamos con casi 5000 palabras** (daría para jugar 29 años seguidos aproximadamente, aunque recuerda que **NO significa que haya esa cantidad de palabras en la RAE**, simplemente estamos analizando las palabras usadas de un conjunto amplío de textos), que representa aproximadamente el 7% de nuestro corpus.

**¿Qué sucede con el conjunto de 5 letras?** El **juego WORDLE se reduce a palabras de 5 letras**: ¿cuáles son las palabras más repetidas de dicho tamaño? 
...
En concreto hay **32 palabras de las 620 que no están en nuestro corpus filtrado**: cotar, jurco, lituo, koala, licra, minal, mimar, boxer, koine, liana, cagon, kefir, rimar, sarza, titar, zebra,  finta, tesla, leñar, kilim, misio, kopek, sarro, lemur, cague, arete, catar, rublo, abano, ostia, arepa.
...
Dichas palabras vamos a buscarlas en el corpus original sin eliminar palabras con frecuencias demasiado pequeñas, e incluiremos dichas palabras con sus frecuencias en nuestros corpus filtrado.
...
He aquí una **tabla con la frecuencia de cada letra** en los vocablos del CREA. Las **letras más comunes en CREA** son la `a, e, o, i, r`, y las que menos aparecen son la `k, ñ, w`.
...
[Elena Álvarez Mellado](https://twitter.com/lirondos), experta en lingüística computacional, apuntaba que quizás una **pista o ayuda para adivinar las palabras** sea analizar qué letras suelen encabezar y terminas las palabras en castellano. El tuit es este <https://twitter.com/lirondos/status/1480875293283954693>
...
# Puntuamos palabras
puntuar_palabras <-
  function(palabras, letras_puntuadas, nletras = 5) { 
...
CREA_puntuado <-
  puntuar_palabras(datos_CREA_filtrado %>%
                     filter(nletras == 5),
                   puntuacion_letras_5)
WORDLE_puntuado <- puntuar_palabras(datos_palabras_wordle,
                                    puntuacion_letras_5)
...
simulacion_wordle <-
  function(corpus, puntuacion_letras,
           palabras_candidatas = corpus,
    ...
    # Puntuamos palabras
    puntuacion <- 
      puntuar_palabras(corpus, puntuacion_letras)
    corpus_wordle_puntuado <- puntuacion$palabras_puntuadas
    matriz_letras_wordle <- puntuacion$matriz_letras
...
iniciar_equi <- FALSE
simulacion_WORDLE <-
  simulacion_wordle(datos_palabras_wordle,
                    puntuacion_letras_5,
                    simulaciones = simulaciones,
                    generar_equi = generar_equi,
                    iniciar_equi = iniciar_equi)
...
simulacion_mixta <-
  simulacion_wordle(datos_CREA_filtrado %>%
                     filter(nletras == 5),
                    puntuacion_letras_5,
                    palabras_candidatas = datos_palabras_wordle,
                    simulaciones = simulaciones,
                    generar_equi = generar_equi,
                    iniciar_equi = iniciar_equi)
...
palabras_fallo <- palabras_iniciales[intentos_mixta == 7]
...
simulacion_muy_dummy_mixta <-
  simulacion_wordle(datos_CREA_filtrado %>%
                    ...
                    iniciar_equi = iniciar_equi,
                    dummy_random = TRUE,
                    extremely_dummmy = TRUE)
intentos_muy_dummy_mixta <-
  unlist(simulacion_muy_dummy_mixta$resultados["intentos", ])
distrib_intentos_muy_dummy_mixta <-
  100 * table(intentos_muy_dummy_mixta) / simulaciones
media_intentos_muy_dummy_mixta <- mean(intentos_muy_dummy_mixta)
distrib_intentos_muy_dummy_mixta
media_intentos_muy_dummy_mixta
```
</details>
...
tabla_intentos_mixta <- 
  tibble("intentos" = sort(unique(intentos_mixta)),
         ...
         "frecuencia" = distrib_intentos_mixta /
           sum(distrib_intentos_mixta))
tabla_intentos_muy_dummy_mixta <- 
  tibble("intentos" = sort(unique(intentos_muy_dummy_mixta)),
         "veces_aleat" = table(intentos_muy_dummy_mixta),
         "frecuencia_aleat" = distrib_intentos_muy_dummy_mixta /
           sum(distrib_intentos_muy_dummy_mixta))
intentos_conjunto_mixta2 <- left_join(tibble("intentos" = 1:7),
                               tabla_intentos_mixta, by = "intentos")
intentos_conjunto_mixta2 <-
  left_join(intentos_conjunto_mixta2, tabla_intentos_muy_dummy_mixta,
            by = "intentos") %>% 
  mutate(across(everything(), ~replace_na(.x, 0)))

df_mixta2 <- # tabla resultados
  intentos_conjunto_mixta2 %>%
  mutate(intentos = ifelse(intentos == 7, "FALLO",
                           as.character(intentos)))
...
datatable(df_mixta2,
  ...
  formatPercentage(c("frecuencia", "frecuencia_aleat"),
                   digits = 2) %>%
  formatStyle(names(df_mixta2[, c(2, 4)]),
              background =
                styleColorBar(range(df_mixta2[, c(2, 4)]), "#c9b458"),
              backgroundSize = '98% 88%',
              backgroundRepeat = 'no-repeat',
              backgroundPosition = 'center') %>%
  formatStyle("intentos",
...
Por último vamos a realizar una busqueda de las **palabras que mejor funcionan como palabra inicial**. Para ello vamos a considerar las **palabras del CREA más repetidas** (que aparezcan en más de 60 de cada 1000 documentos) y añadirle las palabras que sabemos que pueden ser candidatas en WORDLE (y no estuviera). Para cada una vamos a generar un **número de simulaciones y contabilizar el número de éxitos o fracasos**.

<details>
  <summary>📝<strong>Código</strong></summary>

```{r}
datos_CREA_frecuentes <-
  datos_CREA_filtrado %>% filter(frec_norm > 60 & nletras == 5)
datos_CREA_frecuentes <- # añadimos todas las de wordle
  rbind(datos_CREA_frecuentes, datos_palabras_wordle) %>%
  distinct(palabra, .keep_all = TRUE)
...
media_intentos <- mediana_intentos <- n_fallos <- rep(0, simulaciones)
for (i in 1:nrow(datos_CREA_frecuentes)) {
  ...
  simulacion[[i]] <-
      simulacion_wordle(datos_CREA_frecuentes,
                        ...
                        generar_equi = generar_equi,
                        iniciar_equi = iniciar_equi,
                        inicial_fija = datos_CREA_frecuentes$palabra[i],
                        dummy_random = FALSE)
    intentos[[i]] <-
...
```{r echo = FALSE}
df <- tibble("palabra" = datos_CREA_frecuentes$palabra,
...
Las palabras con mejor «rendimiento» han sido `r df %>% arrange(media) %>% slice_head(n = 5) %>% pull(palabra)`.

# Jugar a WORDLE

La idea es hacer en el futuro un simulador del juego en `R` pero...to be continued. Tenéis un simulador en `R` para el juego en inglés en <https://github.com/coolbutuseless/wordle>
...
La principal limitación de este pequeño análisis es mi **propia ignorancia**: no soy experto en minería de datos ni en **procesamiento natural del lenguaje (NLP)**, más allá de un superficial conocimiento para poder impartir docencia en el máster de minería de datos de la UCM. Así que, obviamente, la metodología tiene un mero objetivo pedagógico y lúdico, siendo ampliamente mejorable.
